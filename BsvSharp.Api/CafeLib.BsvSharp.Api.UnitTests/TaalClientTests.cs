...
namespace CafeLib.BsvSharp.Api.UnitTests 
{
    public class TaalClientTests
    {
        ...
        {
            var response = await Api.QueryTransactionStatus(txHash);
        ...
        {
            var mapiClientMock = new Mock<TaalClient>("apiKey");
        ...
        {
            var response = await Api.SubmitTransaction(txRaw);
